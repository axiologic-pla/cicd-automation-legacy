...
helm show values pharmaledgerassoc/quorum-node > $qnValuesPath
...
if test -f $COMPANY_NAME/$NETWORK_NAME/join; then
  ...
  helm upgrade --install --debug --wait --timeout=600s qn-0 pharmaledgerassoc/quorum-node -f $qnValuesPath -f $ghInfoPath -f $qnInfoPath -f $joinNetworkInfo -f $TMP_FOLDER_PATH/deployment.yaml -f $TMP_FOLDER_PATH/networkName.yaml -f $updatePartnersInfo --set-file use_case.joinNetwork.plugin_data_common=$TMP_FOLDER_PATH/join-network.plugin.json,use_case.joinNetwork.plugin_data_secrets=$TMP_FOLDER_PATH/join-network.plugin.secrets.json,use_case.updatePartnersInfo.plugin_data_common=$TMP_FOLDER_PATH/update-partners-info.plugin.json
#  rm -rf $TMP_FOLDER_PATH
  ...
  helm upgrade --install --debug --wait --timeout=600s qn-0 pharmaledgerassoc/quorum-node -f $qnValuesPath -f $ghInfoPath -f $qnInfoPath -f $newNetworkService -f $TMP_FOLDER_PATH/deployment.yaml -f $TMP_FOLDER_PATH/networkName.yaml -f $updatePartnersInfo --set-file use_case.newNetwork.plugin_data_common=$TMP_FOLDER_PATH/new-network.plugin.json,use_case.newNetwork.plugin_data_secrets=$TMP_FOLDER_PATH/new-network.plugin.secrets.json,use_case.updatePartnersInfo.plugin_data_common=$TMP_FOLDER_PATH/update-partners-info.plugin.json
#  rm -rf $TMP_FOLDER_PATH
fi